...
// Zentrale Verwaltung: ein Datensatz je Sitz, mit Hersteller,
// Bemerkung, letztem Check.
//
// Diese Liste (sitze:list) ist die FÜHRENDE Tabelle für Sitze:
// Material-Einträge mit Art="Sitz" verweisen per ID hierher (Feld
// sitzId im Material-Eintrag), nie umgekehrt — siehe material.jsx/
...
function SitzeApp() {
  ...
  const load = React.useCallback(async () => {
    ...
    let stored = [];
    try {
      const r = await window.storage.get(SITZE_KEY);
  ...
  const saveList = async (next) => {
    setList(next);
    try { await window.storage.set(SITZE_KEY, JSON.stringify(next)); } catch {}
    markDirty();
  };

  // Legt für jeden Material-Eintrag mit Art="Sitz" einen Eintrag an,
  // falls noch keiner mit passendem Namen (= Material-Spalte "Typ")
  // existiert, und repariert danach die sitzId-Verknüpfung bei allen
  // betroffenen Material-Einträgen.
  const generateFromMaterial = async () => {
    setBusy(true); setMsg(null);
    try {
      const mat = material || await loadAllMaterial();
      const relevant = mat.filter(m => (m.art || "").trim() === SITZ_ART);
      ...
      for (const m of relevant) {
        ...
        const entry = { id: `sitz_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
      ...
      for (const m of relevant) {
        ...
        if (target && m.sitzId !== target.id) {
          const updated = { ...m, sitzId: target.id };
    ...
    <div style={{ minHeight: "100vh", background: "#040e20", color: "#e8f4fd", fontFamily: "system-ui,sans-serif", paddingBottom: 40 }}>
      <div style={{ padding: "calc(18px + env(safe-area-inset-top, 0px)) 16px 6px", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>🎽 Sitze</h1>
      ...
      <div style={{ padding: "0 16px 14px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <button onClick={generateFromMaterial} disabled={busy}
          title="Legt für jeden Material-Eintrag mit Art=&quot;Sitz&quot; einen Eintrag an, falls noch keiner existiert, und repariert alle Verknüpfungen."
      ...
      <div style={{ padding: "0 16px" }}>
        {sorted.length === 0 && (
          <div style={{ padding: "30px 12px", textAlign: "center", fontSize: 13, color: "rgba(232,244,253,0.4)" }}>
            Noch keine Einträge — über „+ Neuer Eintrag" anlegen, oder „🔄 aus Material synchronisieren", sobald Material-Einträge mit Art="Sitz" vorhanden sind.
          </div>
        )}
        {sorted.map(entry => {
          ...
          return (
            <div key={entry.id} onClick={() => setEditing(entry)}
              ...
              <div style={{ fontSize: 12, color: "rgba(232,244,253,0.7)", display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                ...
                {matList.length > 0 && (
                  <a href={`material.html?sitzId=${entry.id}`} onClick={e => e.stopPropagation()} style={{ color: "rgba(74,222,128,0.8)", textDecoration: "none" }}>🔗 {matList.length} Material-Eintrag(e)</a>
                )}
              </div>
            </div>
          );
      ...
      {editing && (
        <GruppeEditor
          ...
          onSave={async (data) => {
            ...
            const isNew = !data.id;
            if (isNew) {
              const entry = { ...data, id: `sitz_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, name: trimmedName };
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 14, marginBottom: 8 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              {materialList.length > 0 && (
                <a href={`material.html?sitzId=${entry.id}`} style={{ fontSize: 11, color: "#7dd3fc", textDecoration: "none" }}>→ in Material öffnen</a>
              )}
            </div>
            {materialList.length === 0 && (
